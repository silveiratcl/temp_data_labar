...
temp_data <- list.files(path = "data",
                   pattern = "*.xlsx",
                   full.names = TRUE,
                   recursive = TRUE) %>% # 1 read data folder# 2 read data inside all locations
  
  lapply(read_excel(col_types = "text")) %>% 
  bind_rows # 3 rbind all data

# https://stackoverflow.com/questions/51929462/specify-col-type-by-name-in-lapply
...
read_excel("temp_tidybit_sv_2022-08-17.xlsx",
           col_types = c("date", 
                         "text",
                         "numeric", 
                         "numeric", 
